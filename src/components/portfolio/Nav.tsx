        ...
        <nav className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="text-[14px] font-body text-(--color-text-muted) hover:text-(--color-text) transition-colors duration-150 no-underline"
            >
              {link.label}
            </a>
          ))}
          ...
          >
            {navLinks.map((link) => (
              <a
                ...
                className="flex items-center h-12 pl-6 text-[14px] font-body text-(--color-text-muted) hover:text-(--color-text) transition-colors duration-150 no-underline"
              >
                {link.label}
              </a>
            ))}
